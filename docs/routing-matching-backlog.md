...
> **Nachgezogen (2026-08-25) — Laufzeit-Degradierung sichtbar gemacht:** 1.1 deckte nur die *Fehlkonfiguration* laut ab; ein Straßen-Provider, der zur Laufzeit ausfällt, wurde vom `FallbackRoutingProvider` weiterhin still auf Luftlinie umgeleitet (nur `console.warn`). Damit plante der Dienst mit Schätzwerten, die er für echte Fahrzeiten hielt. Neu: `routing/degradierung.ts` (Meldung ins Log + entprellt an Sentry, ein Event je Grund/5 Min) und `routing/status.ts` (`pruefeRouting()` fragt den **primären** Provider aktiv mit zwei festen Testpunkten — nie Klientendaten —, 60 s gecacht; der Fallback würde den Ausfall sonst maskieren). Ausgewiesen in `GET /api/v1/health` (`routing.modus` + `grund`; bewusst **kein** 503, die App plant ja weiter) und als Banner auf Tourenplanung und Berichten (`RoutingHinweis.tsx`, i18n de+en). Env-Duplikation in `routing/konfig.ts` zusammengezogen; der stille Geometrie-Fallback in `routeGeometrie.ts` meldet jetzt ebenfalls. 14 neue Tests.
...
**Umsetzung:** Reine Aggregation `server/berichte/` — `aggregiereBerichte()` gruppiert je Tour berechnete Kennzahlen nach `pflegekraftId` → **Mitarbeiterauslastung** (Touren, Einsätze, Arbeits-/Fahr-/Klientenzeit, Auslastung % = amKlienten/(amKlienten+Fahrzeit), km) und **Kilometernachweis** (je Tour). Service `berechneBerichte()` lädt die Touren des Zeitraums und rechnet je Tour über `planeTour` (Zeit) + `berechneTourKm` (km). Endpoints `GET /api/v1/berichte` (JSON) + `/berichte/csv?typ=auslastung|kilometer` (CSV/BOM), Rollen disponent/admin. UI-Seite `/berichte` (Zeitraum, zwei Tabellen, CSV-Downloads) + Dashboard-Link; i18n de+en.

**Echte Straßen-km (Nachtrag 2026-07-28):** Der `RoutingProvider`-Port um optionales `distanzMatrix()` (km) erweitert — implementiert in Haversine (geometrisch), OSRM (`annotations=distance`), HERE (`matrixAttributes: ['distances']`), mit Fallback- und Cache-Wrapper. `berechneTourKm()` summiert die echten Streckenlängen aus der Distanzmatrix des aktiven Providers (`summeKm`, rein/getestet) — bei OSRM/HERE **echte Straßen-km**, mit Haversine weiterhin geometrische Schätzung. 8 Unit-Tests (km-Geometrie, `summeKm`, Aggregation/Auslastung, CSV, OSRM-/HERE-`distanzMatrix`).

> **Bewusst NICHT Teil:** Auslastung gegen Vertragsarbeitszeit (nicht erfasst). Qualitätsindikatoren (Pünktlichkeit) deckt der Soll-Ist-Abgleich (2.4) bereits ab.
...
**KBV-FHIR-Mapping (Nachtrag 2026-07-28):** Toleranter Subset-Parser `ti/fhir.ts::mappeFhirBundle()` — liest aus einem FHIR-Bundle Patient (Name/Adresse/KVNR über `kvid`-System), ServiceRequest (Leistungscodes + `occurrencePeriod`) und Practitioner/Organization (LANR über `KBV_NS_Base_ANR`) → `EvoNutzlast`; Leistungscode-Crosswalk `leistungsCodes.ts` (Beispiel + Pass-through). Der Endpoint erkennt `resourceType=Bundle` automatisch; beide Formate laufen durch dieselbe `evoSchema`-Validierung. 4 Unit-Tests (gültiges Bundle → Mapping, fehlendes Bundle/ServiceRequest/Zeitraum → Fehler).

> **Bewusst NICHT Teil (klar markiert):** echter Konnektor/SMC-B/TLS, gematik-Zulassung, **KBV-FHIR-Vollprofil-Konformität** (StructureDefinition-/Terminologie-Validierung — Subset-Parser vorhanden), offizieller Leistungscode-Crosswalk (regionaler Katalog), KIM vollständig (S/MIME über TI), echter ePA-Zugriff (VSDM/Einwilligung), Patienten-Dedup über Versichertennummer (KVNR wird als externalId schon mitgeführt).
