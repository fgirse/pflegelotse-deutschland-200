...
import { ladeAbwesenheitPredikat } from '@/server/abwesenheit/service'
...
// bleiben erhalten, nur fehlende Tage werden ergänzt. Abwesende Kräfte (Urlaub/
// Krankheit) werden an den betroffenen Tagen übersprungen.
export async function generiereWochenplan(tenantId: string, datum: string) {
  ...
  const istAbwesend = await ladeAbwesenheitPredikat(tenantId)

  const alle = generiereWoche(stammtouren, montag, istAbwesend)
  // Ohne Abwesenheiten zur Zählung, wie viele Tage deswegen entfielen.
  const wegenAbwesenheit = generiereWoche(stammtouren, montag).length - alle.length
